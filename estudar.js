// Banco de dados contendo todas as 80 questões rigorosamente revisadas conforme o documento original
const questoesDB = [
    ...
    { id: 45, tipo: 'objetiva', pergunta: "Como o projeto 2 trata erros de validação de forma centralizada?", opcoes: ["Inserindo explicitamente enormes e custosos blocos manuais detalhados compostos em forma de try/catch espalhados visíveis em cada método presente da controllers API.", "Redirecionando rudimentar de forma imediata o frontend chamador e o browser logado direto na raiz rumo a uma página morta estática padrão configurada previamente HTML que simula o erro visual.", "Usa GlobalExceptionHandler com @RestControllerAdvice. Captura MethodArgumentNotValidException e retorna mapa com erros de cada campo com status 400.", "Ocultando silenciosamente a grande maioria pontual em logs e ignorando silenciosamente passivamente as demais falhas ocorridas durante requisição."], correta: 2 },
    { id: 46, tipo: 'objetiva', pergunta: "O que é Spring Security e qual é sua função nos projetos 2 e 3?", opcoes: ["Trata-se explicitamente unicamente isolado em si ser um popular sistema rígido pesado atuante tipo um estrito antivírus adaptável desenhado apenas em atuar na JVM.", "Módulo padrão do Spring para autenticação e autorização. Projeto 2 usa sessões HTTP; projeto 3 usa JWT stateless.", "É a interface focada na restrição avançada imposta do isolamento da robusta forte camada de criptografia em discos nativos.", "Funciona exclusivamente focado operante num poderoso Firewall fechado forte contido integrado empacotado acoplado nativamente na engine Tomcat."], correta: 1 },
    { id: 47, tipo: 'objetiva', pergunta: "Por que nunca armazenar senhas em texto puro? Qual algoritmo o livro usa?", opcoes: ["Apesar das senhas expostas se o banco for comprometido não causarem problema, o livro obriga uso direto em implementações com MD5 rudimentar.", "Para garantir integridade em trânsito de rede. Usa SHA-1 nativo simplório legível estrito.", "Senhas expostas se o banco for comprometido. O livro usa BCrypt via PasswordEncoder, que gera hashes resistentes a força bruta.", "Senhas em texto puro são pesadas. O livro recomenda AES-256 Nativo com chave rígida local."], correta: 2 },
    { id: 48, tipo: 'objetiva', pergunta: "O que é o UserDetailsService e como é implementado no projeto 2?", opcoes: ["É classificado apenas como um importante e restrito serviço interno embutido focado na renderização de templates HTML antigos.", "Interface que o Spring Security usa para carregar usuários por username. A implementação busca por email no banco e retorna User com roles.", "Trata-se do restrito automatizado módulo rotineiro serviço agendado rodando em contínuo envio automático voltado a disparos de emails.", "Consiste no restrito formato de log ou uma tabela de uso secundária oculta operante ativa auxiliar para auditoria."], correta: 1 },
    { id: 49, tipo: 'objetiva', pergunta: "Como o projeto 2 configura rotas públicas e protegidas?", opcoes: ["Via arquivos XML antigos e estáticos que demandam reinicialização completa do Servlet.", "Utilizando a anotação @Public nas controllers, ignorando o uso de filtros de segurança.", "Via SecurityFilterChain com authorizeHttpRequests. Rotas de autenticação e leitura de posts são permitAll. Criar, editar e excluir requerem authenticated.", "Através de middlewares diretos configurados no Nginx ou gateway nativo."], correta: 2 },
    { id: 50, tipo: 'objetiva', pergunta: "O que é CORS e como o projeto 2 o configura?", opcoes: ["Cross-Origin Resource Sharing controla quais origens acessam a API. O projeto 2 configura via CorsConfigurationSource, permitindo localhost:8080 e localhost:3000.", "Um protocolo embutido restrito voltado à criptografia avançada complexa automática em fluxos de vídeo.", "Sistema de gerenciamento focado do banco robusto para gestão do denso pool embutido de conexões estáticas.", "Trata-se exclusivamente de ferramenta voltada a atuar na complexa estrutura ativa das redes de gestão isolada na rotas front-end React."], correta: 0 },
    { id: 51, tipo: 'objetiva', pergunta: "O que é JWT e como funciona no projeto 3?", opcoes: ["É a antiga representação base da sessão tradicional estática legada baseada em cookies físicos no servidor backend.", "JSON Web Token assinado digitalmente. O usuário faz login, o backend gera JWT; o frontend armazena e envia em cada requisição no header Authorization: Bearer token.", "Trata-se de uma moderna biblioteca dependência embutida dedicada a templates HTML.", "Configura-se essencial unicamente em um componente de rotas isoladas longas do React."], correta: 1 },
    { id: 52, tipo: 'objetiva', pergunta: "Qual é a diferença entre autenticação por sessão e JWT stateless?", opcoes: ["Na sessão o backend guarda estado estático persistente no banco de dados e JWT opera apenas gerindo validação feita no frontend.", "Com sessões o backend guarda estado em memória. Com JWT stateless toda informação está no token, permitindo escalar horizontalmente sem compartilhar estado.", "O JWT é classicamente lerdo e engessado, com alto déficit de segurança se comparado às sessões.", "Na autenticação por sessão tradicional não se faz uso de cookies do navegador, enquanto o JWT obriga o uso de cookies estáticos."], correta: 1 },
    { id: 53, tipo: 'objetiva', pergunta: "Por que o JWT não deve conter dados sensíveis no payload?", opcoes: ["Pelo motivo de gerar overhead e inchar o payload a um tamanho insustentável para o protocolo HTTP.", "JWT não é criptografado, apenas assinado. Qualquer pessoa que intercepte o token consegue ler o conteúdo usando jwt.io.", "Porque a integração via Spring Security ativamente veta e proíbe recusando em tempo o uso de strings.", "Devido ao rigoroso limite focado onde JWT restrito nativo expira de forma obrigatória em exatos 2 minutos."], correta: 1 },
    { id: 54, tipo: 'objetiva', pergunta: "Qual biblioteca Java é usada para manipular JWT no projeto 3?", opcoes: ["A famosa biblioteca nativa incorporada de Auth0 Secure.", "A dependência padronizada avançada Nimbus JOSE JWT focada em enterprise.", "A biblioteca JJWT (io.jsonwebtoken), adicionada no pom.xml com três artefatos: jjwt-api, jjwt-impl e jjwt-jackson.", "O plugin nativo embutido obrigatório estrito do Spring Auth Token JWT."], correta: 2 },
    { id: 55, tipo: 'objetiva', pergunta: "Qual é o tempo de expiração do JWT no projeto 3 e como é configurado?", opcoes: ["1 hora engessada configurada direto nas variáveis do código fonte embutido em hardcode.", "86400000 milissegundos (24 horas), configurado em app.jwt.expiracao-ms no application.yml. A chave secreta deve vir de variável de ambiente em produção.", "Longo e fixo num período fixado contínuo estrito durável em 7 dias letivos no backend.", "Possui propriedade nativa atípica onde nunca expira isolado sozinho por tempo."], correta: 1 },
    { id: 56, tipo: 'objetiva', pergunta: "O que é React e qual é sua principal ideia arquitetural?", opcoes: ["Poderoso framework focado essencial puramente no desenvolvimento de CSS puro engessado restrito a aplicações mobile.", "Biblioteca JavaScript para interfaces de usuário. Ideia principal: dividir a UI em componentes reutilizáveis com estado próprio e ser declarativo.", "Linguagem programação pesada isolada compilada estrita estruturada complexa focada em operações de backend.", "Robusto banco de dados isolado estático armazenado em cache browser do cliente."], correta: 1 },
    { id: 57, tipo: 'objetiva', pergunta: "O que é uma SPA e como o projeto 3 a implementa?", opcoes: ["Tradicional clássica modelo de antigo tradicional modelo de aplicação baseada em múltiplas Páginas renderizado no backend tradicional com PHP.", "Single Page Application carrega a estrutura inicial e atualiza dinamicamente. Projeto 3 usa React com Vite na porta 5173, separado do backend Spring Boot.", "Poderoso leve robusto utilitário exclusivo proxy embutido nativo de otimização de imagens pesadas no frontend.", "Restrito componente estático focado em armazenar e persistir cache em disco nativo para consultas de rotinas offline."], correta: 1 },
    { id: 58, tipo: 'objetiva', pergunta: "O que é o hook useState e como é exemplificado no livro?", opcoes: ["Função assíncrona pura embutida voltada na chamada requisições pesadas nativas do cliente gerando chamadas puras REST HTTP.", "Gerencia estado local em componentes funcionais. Exemplo: const [valor, setValor] = useState(0) retorna o estado atual e uma função para atualizá-lo.", "Consiste complexo e denso avançado script nativo que impõe severamente bloqueia fluxo e valida formulários HTML base.", "Trata-se do clássico puro recurso que intercepta e o DOM raiz ativamente diretamente puros e afeta estritamente no processamento puros."], correta: 1 },
    { id: 59, tipo: 'objetiva', pergunta: "Qual ferramenta de build é usada no projeto 3 para o frontend React?", opcoes: ["O clássico e complexo Webpack isolado contendo plugins avançados estritos de compilação restrita de forma manual.", "A estrutura fechada monolítica puramente do Create React App (CRA).", "Vite, que serve a aplicação React de forma extremamente rápida na porta 5173 por padrão em desenvolvimento.", "Babel puro acoplado isoladamente rodando num container sem o uso de quaisquer bundlers dinâmicos."], correta: 2 },
    { id: 60, tipo: 'objetiva', pergunta: "O que são rotas protegidas no frontend React?", opcoes: ["Rotas severamente operadas bloqueadas tratadas restritas exclusivamente pelo controle do firewall do backend em camadas fixas.", "Rotas que redirecionam o usuário não autenticado para o login. No projeto 3 protegem checkout, histórico de pedidos e painel administrativo.", "Caminhos fixos rígidos de URLs ofuscados puramente locais na camada nativa restrita de rotas base criptografadas.", "Consistem puramente isolados em uso operante nativos ativados em atuar como caminhos fechados de links absolutos HTTPS."], correta: 1 },
    ...
    { id: 62, tipo: 'objetiva', pergunta: "Quais são os filtros disponíveis na interface do Gerenciador de Tarefas do Projeto 1?", opcoes: ["Quatro filtros engessados compostos de filtragem por Data, Nome e Idade.", "Uso de filtro operando puramente em status de Urgente, Importante e Normal.", "Três filtros via tabs: Todas, Pendentes e Concluídas. Fazem requisições à API com o parâmetro de status correspondente.", "Um único filtro focado em exibir ativas e inativas."], correta: 2 },
    { id: 63, tipo: 'objetiva', pergunta: "Como o Projeto 1 implementa a persistência de dados?", opcoes: ["Arquiva nativamente os dados em arquivos JSON locais.", "Efetua gravação usando matrizes globais estáticas na memória.", "Dados armazenados no PostgreSQL. Cada operação no front faz requisição à API REST via pool de conexões.", "Toda a persistência utiliza o LocalStorage persistente do browser do cliente."], correta: 2 },
    { id: 64, tipo: 'objetiva', pergunta: "O que é a função escapeHtml() usada no frontend e por que é importante?", opcoes: ["Consiste em utilitário para formatar blocos e deixar o texto colorido.", "Recurso que comprime as imagens pesadas renderizadas nas tags HTML nativas.", "Converte caracteres especiais HTML em entidades seguras. Previne ataques XSS de injeção de scripts no DOM.", "Módulo que limpa o cache ativo do navegador do cliente."], correta: 2 },
    ...
    { id: 68, tipo: 'objetiva', pergunta: "O que é um slug e como o Projeto 2 o gera automaticamente?", opcoes: ["Trata-se de código hexadecimal exclusivo gerado no servidor.", "Versão amigável para URL do título. O PostService usa Normalizer para remover acentos, converter para minúsculas e substituir espaços por hífens.", "É referindo a imagem gerada em miniatura de avatar.", "Formato nativo de backup automático gerencial."], correta: 1 },
    { id: 69, tipo: 'objetiva', pergunta: "Como o Projeto 2 implementa paginação nos posts?", opcoes: ["Cortes de paginação ativados em arrays JS gigantes do frontend.", "Usando comandos nativos de LIMIT puro fixos dentro de strings JDBC.", "Com a interface Pageable do Spring Data aliada à anotação @PageableDefault nos controllers.", "Loops infinitos assíncronos via WebSockets constantes."], correta: 2 },
    { id: 70, tipo: 'dissertativa', pergunta: "Quais são as funcionalidades do sistema de blog do Projeto 2?", respostaExata: "Cadastro e login com Spring Security, perfil com bio e avatar, criação/edição de posts (pelo autor), listagem pública, comentários, tags, paginação e busca por título." },
    { id: 71, tipo: 'dissertativa', pergunta: "Quais são as tecnologias do Projeto 3?", respostaExata: "Backend: Java com Spring Boot e JWT. Frontend: React com Vite (usando Axios e React Router). Banco: PostgreSQL." },
    { id: 72, tipo: 'dissertativa', pergunta: "Quais são as funcionalidades principais do e-commerce do Projeto 3?", respostaExata: "Catálogo paginado com busca/filtros, carrinho persistido no backend, checkout, autenticação JWT, rotas protegidas, painel admin e histórico de pedidos do cliente." },
    { id: 73, tipo: 'objetiva', pergunta: "Quais são os status possíveis de um pedido no Projeto 3?", opcoes: ["NOVO, AVALIADO, REJEITADO e CONCLUÍDO.", "PENDENTE, PAGO, ENVIADO, ENTREGUE e CANCELADO. Definidos em um enum Status no backend.", "INICIO, TRAMITE, DESPACHO e FINAL.", "ETAPA_1 a ETAPA_5 numéricos estritos."], correta: 1 },
    { id: 74, tipo: 'dissertativa', pergunta: "Qual é a diferença arquitetural entre o Projeto 2 e o Projeto 3 no frontend?", respostaExata: "No projeto 2 o Spring Boot serve arquivos HTML estáticos. No projeto 3 o React é uma SPA independente rodando na porta 5173 e comunicando via REST JSON (JWT) com a API na porta 8080." },
    { id: 75, tipo: 'objetiva', pergunta: "O que é o painel administrativo do Projeto 3 e quem pode acessá-lo?", opcoes: ["Painel público de relatórios abertos gerenciais.", "Área exclusiva para gerenciar produtos e pedidos, acessível somente a usuários logados com a 'role' ADMIN. Protegido em ambos front e back-end.", "Painel de controle direto sobre as views do banco de dados relacional.", "Espaço de acesso liberado para qualquer conta devidamente autenticada."], correta: 1 },
    { id: 76, tipo: 'dissertativa', pergunta: "O que é Inversão de Controle (IoC) e Injeção de Dependência no Spring Boot?", respostaExata: "IoC: O framework toma o controle do ciclo de vida dos objetos (Beans). Injeção de Dependência: O Spring fornece/injeta automaticamente essas dependências (via construtor ou @Autowired)." },
    { id: 77, tipo: 'objetiva', pergunta: "O que é escalabilidade horizontal e como o JWT a facilita?", opcoes: ["Aumentar memória e CPU de uma máquina única vertical.", "Particionar tabelas do banco em múltiplos discos de storage isolado.", "Adicionar mais instâncias de servidor. JWT facilita por ser stateless: toda a informação de autenticação vai no token, qualquer instância pode atender sem consultar sessões.", "Gerar estatísticas de crescimento horizontal através de métricas de acesso nativas do token."], correta: 2 },
    { id: 78, tipo: 'objetiva', pergunta: "O que é @RestControllerAdvice e qual é seu papel no projeto 2?", opcoes: ["Anotação que programa desligamentos de rotas com anomalias.", "Cria um serviço de logs de disco severo.", "Anotação que define tratamento global de exceções. Captura erros e retorna respostas HTTP JSON padronizadas (ex: 400 Bad Request) sem poluir os controllers com try/catch.", "Avisa administradores enviando logs via protocolo SMTP nativo da nuvem."], correta: 2 },
    { id: 79, tipo: 'objetiva', pergunta: "O que é Flyway ou Liquibase e por que o livro os menciona para produção?", opcoes: ["Plataformas em nuvem robustas.", "Componentes CSS nativos para animação fluida.", "Frameworks baseados em controle multithread intensivo nativo.", "Ferramentas de migration que versionam alterações de schema (tabelas). Em produção são essenciais pois evitam a perda de dados causada pelo ddl-auto: update do Hibernate."], correta: 3 },
    { id: 80, tipo: 'dissertativa', pergunta: "Qual é a lição pedagógica do livro ao apresentar três projetos com complexidade crescente?", respostaExata: "Mostrar de forma prática como os mesmos conceitos (CRUD, banco, autenticação) se aplicam e evoluem, passando de Node.js + HTML simples, para o robusto MVC Java e culminando em SPA React + Spring Boot + JWT." }
];

const quizContainer = document.getElementById('quiz-container');

// Embaralha as alternativas das objetivas
...
// Renderiza a questão objetiva para estudo
...
// Renderiza a questão dissertativa para estudo
function renderDissertativa(q, index) {
    ...
    card.innerHTML = `
        ...
        <h3 class="question-title">${q.pergunta}</h3>
        <textarea class="dissertativa-input" placeholder="Pense e escreva sua resposta aqui antes de conferir..."></textarea>
        <button class="action-btn btn-revelar">Ver Gabarito Completo</button>
        <div class="gabarito-box hidden">
            <strong>Gabarito Oficial Esperado:</strong> <br/><br/>
            ${q.respostaExata}
        </div>
    `;
    return card;
...
// Escuta de cliques globais (Validação e Mostrar Gabarito)
quizContainer.addEventListener('click', (e) => {
    // Lógica para questões objetivas
    if (e.target.classList.contains('option-btn')) {
        ...
        botoes.forEach(b => {
            b.disabled = true; // Impede novos cliques
    ...
    // Lógica para revelar o gabarito das questões dissertativas
    if (e.target.classList.contains('btn-revelar')) {
        const btn = e.target;
        const gabaritoBox = btn.nextElementSibling;
        gabaritoBox.classList.remove('hidden');
        btn.classList.add('hidden'); // Some com o botão de revelar
    }
});

// Inicialização: Renderiza todo o banco formatado
function iniciarModoEstudo() {
